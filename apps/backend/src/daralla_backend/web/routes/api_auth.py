...
def create_blueprint(bot_app):
    ...
    async def api_auth_register():
        ...
        try:
            ...
            auth_data = await register_user_and_issue_token(username, password_hash)
            ...
            logger.warning("Ошибка регистрации (service): %s", e.message)
    ...
    async def api_auth_login():
        ...
        try:
            ...
            auth_data = await login_user_and_issue_token(username, password)
            token = auth_data["token"]
            ...
            logger.warning("Ошибка входа (service): %s", e.message)
            return jsonify({"error": e.message}), e.status_code
        except ValueError as e:
    ...
    async def api_auth_verify():
        ...
        try:
            ...
            logger.warning("Ошибка verify (service): %s", e.message)
            return jsonify({"error": e.message}), e.status_code
