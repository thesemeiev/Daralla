...
- **Сумма:** в USD. Конвертация: `amount_usd = amount_rub / CRYPTOCLOUD_RUB_TO_USD` (env, по умолчанию 100), минимум 0.01 USD, округление до 2 знаков.
- **Тело запроса:** `amount` (USD), `currency: "RUB"`, `shop_id`, `order_id` (внутренний ID платежа), `add_fields`:
...
**Переменные окружения:** `CRYPTOCLOUD_API_KEY`, `CRYPTOCLOUD_SHOP_ID`, `CRYPTOCLOUD_RUB_TO_USD` (опционально).
...
1. **Env:** `CRYPTOCLOUD_API_KEY`, `CRYPTOCLOUD_SHOP_ID`, при необходимости `CRYPTOCLOUD_RUB_TO_USD` и в проде `CRYPTOCLOUD_WEBHOOK_SECRET`.
