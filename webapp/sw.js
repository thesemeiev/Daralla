        ...
        return caches.match('/offline.html').then(function (offline) {
          return offline || caches.match(event.request).then(function (cached) {
            return cached || new Response(
              '<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width"><title>Нет соединения</title></head><body style="font-family:sans-serif;background:#1a1a1a;color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;padding:24px;text-align:center"><h1>Нет соединения</h1><p>Проверьте сеть и обновите страницу.</p><button onclick="location.reload()" style="padding:12px 24px;margin-top:16px;background:#4a9eff;color:#fff;border:none;border-radius:8px;cursor:pointer">Обновить</button></body></html>',
