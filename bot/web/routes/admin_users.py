...
def create_blueprint(bot_app):
    ...
    async def api_admin_user_create_subscription(request, admin_id, user_id_param):
        ...
        logger.info(
            "Подписка создана в RemnaWave runtime: subscription_id=%s, user_id=%s, period=%s",
            subscription_id,
            user_id_resolved,
            period,
        )
    ...
    async def api_admin_user_delete(request, admin_id, user_id_param):
        ...
        remna_service = get_ctx().remnawave_service
        if remna_service:
            for sub in all_subscriptions:
                try:
                    await remna_service.suspend_access(subscription_id=sub["id"])
                except Exception as e:
                    logger.warning("Ошибка suspend_access для subscription=%s: %s", sub["id"], e)
