...
def create_subscription_blueprint(bot_app):
    ...
    async def subscription(token):
        ...
        try:
            ...
            link = await subscription_manager.get_subscription_link(sub["id"])
            ...
            try:
                usage = await get_ctx().remnawave_service.get_usage(subscription_id=sub["id"])
                total_upload = int(usage.get("upload") or 0)
                total_download = int(usage.get("download") or 0)
                total_traffic = int(usage.get("total") or 0)
            except Exception as e:
                logger.warning("Ошибка получения статистики трафика RemnaWave: %s", e)
