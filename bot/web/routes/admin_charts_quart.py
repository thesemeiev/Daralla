...
def create_blueprint(bot_app):
    ...
    async def api_admin_charts_user_growth():
    ...
    async def api_admin_charts_conversion():
        if request.method == "OPTIONS":
            return "", 200, CORS_HEADERS
        try:
            _, err = await require_admin(request)
    ...
    async def api_admin_charts_notifications():
        if request.method == "OPTIONS":
            return "", 200, CORS_HEADERS
        try:
            _, err = await require_admin(request)
    ...
    async def api_admin_charts_subscriptions():
        if request.method == "OPTIONS":
            return "", 200, CORS_HEADERS
        try:
            _, err = await require_admin(request)
            ...
            logger.error("Ошибка в /api/admin/charts/subscriptions: %s", e, exc_info=True)
            return jsonify({"error": "Internal server error"}), 500, _cors_headers()
