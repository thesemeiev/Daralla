                ...
                try:
                    ...
                    if len(active_subs) == 0:
                        ...
                        if not subscription_manager:
                            raise RuntimeError("subscription_manager unavailable")
                        sub_dict, _token = await subscription_manager.create_subscription_for_user(
                            user_id=user_id,
                            period="month",
                            device_limit=1,
                            price=0.0,
    ...
    async def api_user_server_usage():
        ...
        try:
            ...
            if not user_id:
                return jsonify({"error": "Invalid authentication"}), 401
            _ = user_id
            return jsonify({"success": True, "servers": []}), 200, _cors_headers()
        except Exception as e:
